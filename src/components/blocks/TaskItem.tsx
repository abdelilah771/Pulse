...
export default function TaskItem({ task, creator, assignees = [], projects = [] }: TaskItemProps) {
    ...
    return (
        <>
            <div className={`glass-card p-5 rounded-3xl relative overflow-hidden group transition-all ${isDone ? "opacity-70 dark:opacity-50" : ""} ${priority.border}`}>
                {/* Left accent bar */}
                ...
                {/* Title */}
                <h4 className={`text-lg font-bold mb-1 transition-colors ${isDone ? "line-through text-slate-500 dark:text-slate-500" : "text-slate-900 dark:text-white"}`}>
                    {task.text}
                </h4>
